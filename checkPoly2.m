function out = checkPoly(A,B,K,param)
...
elseif out.N==2
    for i=1:n
        out.eig(i,:)=eig(a(i)*(A{1}+B*K)+(1-a(i))*(A{2}+B*K));
        out.alpha(i,1)=a(i);
        out.alpha(i,2)=1-a(i);
    end
elseif out.N>2
    ...
    for j=1:size(L,1)
        for i=1:n
            out.eig(k,:)=eig(a(i)*(A{L(j,1)}+B*K)+(1-a(i))*(A{L(j,2)}+B*K));
